...
# Load processed data
print("Loading processed forward data...")
try:
    df = pd.read_csv('forward_processed.csv')
    print(f"Loaded {len(df)} rows of forward data")
except FileNotFoundError:
    print("Error: forward_processed.csv not found. Please run the feature engineering script first.")
    exit(1)
...
enhanced_features = core_features + [
    'goal_involvement_per_90', 'shots_per_90', 'minutes_consistency',
    'goal_involvement_pct', 'recent_goal_inv_pct', 'home_goals_per_game', 'away_goals_per_game',
    'shot_conversion', 'recent_shot_conversion', 'recent_creativity', 'recent_threat',
    'xG_per_90', 'xA_per_90', 'goals_minus_xG', 'assists_minus_xA',
    'penalty_taker', 'goal_streak', 'next_3_attacking_potential'
]
...
plt.title('Feature Importance for Forward Points Prediction')
plt.tight_layout()
plt.savefig('forward_feature_importance.png')
print("Feature importance plot saved to 'forward_feature_importance.png'")
...
plt.title('Forward Model: Predicted vs Actual Points')
plt.tight_layout()
plt.savefig('forward_prediction_accuracy.png')
print("Prediction accuracy plot saved to 'forward_prediction_accuracy.png'")

# Save the model
print("\nSaving model to 'forward_model.pkl'...")
with open('forward_model.pkl', 'wb') as f:
    pickle.dump(final_pipeline, f)

# Save feature list used for training (helpful for prediction)
with open('forward_model_features.txt', 'w') as f:
    f.write('\n'.join(features))
print(f"Feature list saved to 'forward_model_features.txt'")
