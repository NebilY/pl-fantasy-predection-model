...
# Load processed data
print("Loading processed goalkeeper data...")
try:
    df = pd.read_csv('goalkeeper_processed.csv')
    print(f"Loaded {len(df)} rows of goalkeeper data")
except FileNotFoundError:
    print("Error: goalkeeper_processed.csv not found. Please run the feature engineering script first.")
    exit(1)
...
enhanced_features = core_features + [
    'saves_per_game', 'recent_saves', 'save_points_per_game', 'recent_save_points',
    'recent_clean_sheets', 'home_clean_sheet_rate', 'away_clean_sheet_rate',
    'goals_conceded_per_game', 'recent_goals_conceded', 'minutes_consistency',
    'penalties_saved_per_game', 'has_saved_penalty', 'bonus_per_game', 'recent_bonus',
    'clean_sheet_streak', 'xGC_per_game', 'recent_xGC', 'goals_conceded_minus_xGC',
    'save_percentage', 'recent_save_percentage', 'points_per_90', 
    'team_recent_clean_sheets', 'team_recent_goals_conceded',
    'next_3_cs_potential', 'next_3_save_potential'
]
...
plt.title('Feature Importance for Goalkeeper Points Prediction')
plt.tight_layout()
plt.savefig('goalkeeper_feature_importance.png')
print("Feature importance plot saved to 'goalkeeper_feature_importance.png'")
...
plt.title('Goalkeeper Model: Predicted vs Actual Points')
plt.tight_layout()
plt.savefig('goalkeeper_prediction_accuracy.png')
print("Prediction accuracy plot saved to 'goalkeeper_prediction_accuracy.png'")

# Save the model
print("\nSaving model to 'goalkeeper_model.pkl'...")
with open('goalkeeper_model.pkl', 'wb') as f:
    pickle.dump(final_pipeline, f)

# Save feature list used for training (helpful for prediction)
with open('goalkeeper_model_features.txt', 'w') as f:
    f.write('\n'.join(features))
print(f"Feature list saved to 'goalkeeper_model_features.txt'")
